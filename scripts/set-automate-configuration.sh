...
#OSQO 
    ...
    #PEERS
    export OSQO_IP=$(echo $config| jq -r '.machines[] | select(.name == "integra") | .ip')
    export PEER_1_OSQO=$(echo $config| jq -r '.machines[] | select(.name == "integra") | .ports.peer0_port')
    export PEER_2_OSQO=$(echo $config| jq -r '.machines[] | select(.name == "integra") | .ports.peer1_port')
    
 
    #ORDERERS
    export ORDERER_1_OSQO=$(echo $config| jq -r '.machines[] | select(.name == "integra") | .ports.orderer_1_port')



    #COUCHDB
    export COUCHDB_USER_OSQO_PEER1="$(echo $config| jq -r '.machines[] | select(.name == "integra") | .couchDbCreds.couch_peer1.user_name')"
    export COUCHDB_PASSWORD_OSQO_PEER1="$(echo $config| jq -r '.machines[] | select(.name == "integra") | .couchDbCreds.couch_peer1.password')"

    export COUCHDB_USER_OSQO_PEER2="$(echo $config| jq -r '.machines[] | select(.name == "integra") | .couchDbCreds.couch_peer2.user_name')"
    export COUCHDB_PASSWORD_OSQO_PEER2="$(echo $config| jq -r '.machines[] | select(.name == "integra") | .couchDbCreds.couch_peer2.password')"
