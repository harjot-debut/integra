...
function createConsortium() {
  ...
  set -x
  # configtxgen -profile  OrgsOrdererGenesis -channelID ${CHANNEL_NAME} -outputBlock ${PWD}/../system-genesis-file/genesis.block -configPath ${PWD}/../configtx/
  configtxgen -profile  OrgsOrdererGenesis -channelID ${CHANNEL_NAME} -outputBlock ${PWD}/../system-genesis-file/${CHANNEL_NAME}.block 
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
